...
let render_fragment = `
  ...
  void main()
  {
    ...
    vec2 display_pos = (frag2sim * gl_FragCoord.xy) / simulation_resolution;
    vec3 texture_color = texture2D(texture, display_pos).rgb;

    vec3 diffuse_color = mix(background_color, texture_color, foregroundness);
